...
class SMIE:
    def __init__(self, cfg):
        ...
        # load data
        self.tr_event = t.from_numpy(cfg['tr_event']).float()
        self.tr_ind = t.from_numpy(cfg['tr_ind']).long()
        self.te_event = t.from_numpy(cfg['te_event']).float()
        ...
        self.g_B = self.register_param(t.rand((self.induce_nb, 2 * self.embedding_dim * self.entity_typenb + 1)))
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_nb):
            ...
            if epoch % test_every == 0:
                ...
                self.train_mode(True)
            if epoch >= 100 and np.argmax(ll_list) < len(ll_list) - 10:
                break
        return ll_list
    ...
    def get_nELBO(self):
        ...
        fKL = 0.5 * (
            self.tr_mul(t.linalg.solve(fkBB, fLtril), fLtril.transpose(0, 1)) 
            + self.f_b.transpose(0, 1) @ t.linalg.solve(fkBB, self.f_b)
        ...
        gKL = 0.5 * (
            self.tr_mul(t.linalg.solve(gkBB, gLtril), gLtril.transpose(0, 1))
            + self.g_b.transpose(0, 1) @ t.linalg.solve(gkBB, self.g_b)
    ...
    def X(self, ind1, ind2, time1, time2, sample=True, ratio=1.0):
        ...
        if sample:
            ...
            # sample f
            mean = fkXB @ t.linalg.solve(fkBB, f_b)
            var  = 1.0 + self.jitter - (fkXB.transpose(0, 1) * t.linalg.solve(fkBB, fkXB.transpose(0, 1))).sum(0) 
        ...
        x = t.cat((x1.repeat(ind2_nb, 1), x2.repeat_interleave(ind1_nb, dim=0), t_diff), 1)
        gkXB = self.kernel_rbf.cross(x, self.g_B, t.exp(self.g_log_ls))
        gkBB = self.kernel_rbf.matrix(self.g_B, t.exp(self.g_log_ls))

        
        m1 = t.exp(-0.5 * t_diff)
        m2 = t.exp(-0.5 * self.g_B[:, -1]).view((-1, 1))
        ...
        if sample:
            ...
            # sample g
            mean = m1 + gkXB @ t.linalg.solve(gkBB, g_b - m2)
            # print(x.shape)
            var = 1.0 + self.jitter - (gkXB.transpose(0, 1) * t.linalg.solve(gkBB, gkXB.transpose(0, 1))).sum(0)
            std = t.sqrt(var).view(mean.shape)
            epsilon = mean.new(mean.shape).normal_(mean=0, std=1)
            g = mean + std * epsilon
        else:
        ...
        # effect = t.tanh(g) * K * (t.pow(1 + t_diff / t.exp(self.log_dr), -0.5 * (t.exp(self.log_dr) + 1))) * mask
        # effect = t.tanh(g) * K * t.pow(1 + t_diff, -t.exp(self.log_dr)) * mask
        effect = (g.view((ind1_nb, ind2_nb)) * mask).sum(1).view(f.shape) * ratio
