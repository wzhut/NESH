...
class SNETD:
    def __init__(self, cfg):
        ...
        self.log_dr_ls = self.register_param(t.tensor([0.]))
    ...
    def tr_T1(self):
        ...
        for idx in self.inner_event_batch_idx:
            ...
            # print(time_diff)
            dr = self.kernel_rbf.cross(x1, x2, t.exp(self.log_dr_ls))
            # print(dr)
            int_h = beta * dr * (1 - t.exp(-time_diff.view((1, -1)) / dr))
            # print(int_h)
    ...
    def tr_T2(self):
        ...
        beta = t.exp(self.log_beta)
        # dr = t.exp(self.log_dr)
        dr = self.kernel_rbf.pair(x1, x2, t.exp(self.log_dr_ls)).view((outer_event_batch_sz, self.trig_window))
